...
export class ChatService {
  ...
  async getConversations(userId: string): Promise<any[]> {
    ...
    const messages = await this.messageModel.aggregate([
      ...
      { $sort: { createdAt: -1 } },
      { $group: { _id: { $cond: [{ $eq: ['$senderId', userObjectId] }, '$receiverId', '$senderId'] }, lastMessage: { $first: '$$ROOT' } } },
      { $lookup: { from: 'users', localField: '_id', foreignField: '_id', as: 'otherUser' } },
      { $unwind: '$otherUser' },
      { $project: { userId: '$_id', firstName: '$otherUser.firstName', lastName: '$otherUser.lastName', profilePicture: '$otherUser.profilePicture', lastMessage: { content: { $ifNull: ['$lastMessage.content', ''] }, type: '$lastMessage.type', createdAt: '$lastMessage.createdAt' }, lastMessageTime: '$lastMessage.createdAt', unreadCount: { $size: { $filter: { input: { $filter: { input: { $function: { body: `function(messages) { return messages.filter(m => m.receiverId && m.receiverId.toString() === arguments[0] && !m.isRead); }`, args: [userId], lang: 'js' } }, as: 'msg', cond: { $eq: ['$$msg.receiverId', userObjectId] } } }, as: 'msg', cond: { $not: '$$msg.isRead' } } } } } },
  ...
  async getMessages(userId: string, otherUserId: string): Promise<MessageResponseDto[]> {
    if (!Types.ObjectId.isValid(userId) || !Types.ObjectId.isValid(otherUserId)) throw new BadRequestException('ID invalide');
    const userObjectId = new Types.ObjectId(userId), otherObjectId = new Types.ObjectId(otherUserId);
    const messages = await this.messageModel.find({ $or: [{ senderId: userObjectId, receiverId: otherObjectId }, { senderId: otherObjectId, receiverId: userObjectId }] }).sort({ createdAt: 1 }).populate('senderId', 'firstName lastName profilePicture').lean().exec();
    return (messages as any[]).map(msg => ({
      id: msg._id.toString(), senderId: msg.senderId?._id?.toString() || msg.senderId?.toString(), receiverId: msg.receiverId?.toString(),
      type: msg.type, content: msg.content, fileUrl: msg.fileUrl, fileName: msg.fileName, fileSize: msg.fileSize, emoji: msg.emoji,
      isRead: msg.isRead, isDelivered: msg.isDelivered, createdAt: msg.createdAt, moneyTransfer: msg.moneyTransfer,
      sender: msg.senderId ? { id: msg.senderId._id?.toString() || msg.senderId.toString(), firstName: msg.senderId.firstName || 'Utilisateur', lastName: msg.senderId.lastName || '', profilePicture: msg.senderId.profilePicture } : undefined,
    }));
  }
  ...
  async markAsRead(userId: string, senderId: string): Promise<void> {
    ...
    await this.messageModel.updateMany({ senderId: new Types.ObjectId(senderId), receiverId: new Types.ObjectId(userId), isRead: false }, { isRead: true });
  }

  async sendMessage(senderId: string, sendMessageDto: SendMessageDto): Promise<MessageResponseDto> {
    // similaire à saveMessage mais avec validation
