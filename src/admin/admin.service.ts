...
export class AdminService {
  ...
  async getDashboardStats() {
    ...
    const dailyStats = await Promise.all(last7Days.map(async (date) => {
      ...
      const users = await this.userModel.countDocuments({ createdAt: { $gte: date, $lt: endDate } });
      const transactions = await this.transactionModel.countDocuments({ createdAt: { $gte: date, $lt: endDate } });
      const volume = await this.transactionModel.aggregate([
        { $match: { status: TransactionStatus.COMPLETED, createdAt: { $gte: date, $lt: endDate } } },
        { $group: { _id: null, total: { $sum: '$amount' } } }
      ]);
      return { date: date.toISOString().split('T')[0], users, transactions, volume: volume[0]?.total || 0 };
    }));

    const topUsers = await this.transactionModel.aggregate([
      { $match: { status: TransactionStatus.COMPLETED } },
      { $group: { _id: '$senderId', transactionCount: { $sum: 1 }, totalVolume: { $sum: '$amount' } } },
      { $sort: { totalVolume: -1 } },
      { $limit: 5 },
      { $lookup: { from: 'users', localField: '_id', foreignField: '_id', as: 'user' } },
      { $unwind: '$user' },
      { $project: { userId: '$_id', name: { $concat: ['$user.firstName', ' ', '$user.lastName'] }, transactionCount: 1, totalVolume: 1 } }
    ]);

    return { totalUsers, activeUsers, totalTransactions, totalVolume, recentUsers, recentTransactions, dailyStats, topUsers };
  }
  ...
  async getUserById(userId: string) {
    if (!Types.ObjectId.isValid(userId)) throw new NotFoundException('ID utilisateur invalide');
  ...
  async updateUserStatus(userId: string, isActive: boolean) {
    if (!Types.ObjectId.isValid(userId)) throw new NotFoundException('ID utilisateur invalide');
    const user = await this.userModel.findByIdAndUpdate(userId, { isActive }, { new: true }).select('-password');
    if (!user) throw new NotFoundException('Utilisateur non trouvé');
    return user;
  }
  ...
  async getSettings() {
    ...
    if (!settings) {
      settings = await this.settingModel.create({
        general: { siteName: 'SPaye', siteUrl: 'https://spaye.com', adminEmail: 'admin@spaye.com', supportEmail: 'support@spaye.com', maintenanceMode: false, registrationEnabled: true, defaultUserRole: 'user', maxFileSize: 150, sessionTimeout: 30 },
        security: { twoFactorAuth: false, passwordMinLength: 8, passwordRequireUppercase: true, passwordRequireNumbers: true, passwordRequireSpecial: true, maxLoginAttempts: 5, lockoutDuration: 30, sessionTimeout: 60, requireEmailVerification: true, requirePhoneVerification: false },
        payment: { minTransaction: 100, maxTransaction: 5000000, dailyTransferLimit: 5000000, monthlyTransferLimit: 50000000, mobileMoneyEnabled: true, mobileMoneyOperators: { airtel: true, orange: true, mvola: true }, transferFees: { airtel: 0.5, orange: 0.5, mvola: 0.5, internal: 0 }, currency: 'Ar' }
      });
    }
    return settings;
  }
  ...
  async getSystemStats() {
    ...
    } catch (error) { databaseSize = 'Non disponible'; }
    return {
      ...
      activeUsers, totalUsers, databaseSize
    };
  }
